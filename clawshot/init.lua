...
local function register_clawshot(name, desc, texture, texture2, speed, accel, timeout, time2)
	...
	on_step = function(self)		
		...
		if minetest.get_item_group(minetest.get_node(pos).name, "hook") ~= 0 or minetest.get_item_group(minetest.get_node(pos).name, "choppy") ~= 0 or minetest.get_item_group(minetest.get_node(pos).name, "cracky") ~= 0 or minetest.get_item_group(minetest.get_node(pos).name, "crumbly") ~= 0 or  minetest.get_item_group(minetest.get_node(pos).name, "snappy") ~= 0 then 
	...
	on_use = function(item, placer, pointed_thing)
      ...
      click=2
			...
			local obj = {}
      if placer:get_player_control().RMB then
      placer:set_detach()
...
minetest.register_chatcommand("declaw", {
    params = "<text>",
    description = "deactch",
    ...
    then
      ...
      if minetest.player_exists(param) and player:is_player_connected() and player~=nil then
      player:set_detach()
      end
    else 
	...
	minetest.register_tool("clawshot:help_tool", {
	description = "use when stuck",
	inventory_image = "ih.png",
	wield_scale = {x = 1.0, y = 1.0, z = 1.2},
	tool_capabilities = {
		full_punch_interval = 0.7,
		max_drop_level=1,
		groupcaps={
...
register_clawshot("clawshot", "Clawshot", "clawshot.png", "clawshot_claw.png", 10, -6, 1.7, 1)
register_clawshot("clawshot_gold", "Gold Clawshot", "clawshot_gold.png", "clawshot_claw_gold.png", 29, 5, 1.1, 1)
